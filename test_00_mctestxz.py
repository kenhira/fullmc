...
if __name__ == '__main__':
    ...
    radirr = fmc.read_result(kind='irr')

    with open(f'{work_dir}/photon_trajectory.txt', 'r') as fh:
    ...
    ax_cb1.set_title(' '*10 +r'Extinction ($\rm m^{-1}$)', fontsize=10)
    ...
    ax_cb3.set_title(' '*10 + 'weight', fontsize=10, pad=10)

    plt.savefig(f"{work_dir}/00_photon_trajectories_{'ICA' if transfermode == 0 else '3D'}.png", dpi=300)
    plt.close()

    with open(f'{work_dir}/outradirr.txt', 'r') as fh:
        header = fh.readline()  # skip header
    ...
    fig.savefig(f'{work_dir}/00_radirr_xz_downward_y{indy}.png', dpi=300)