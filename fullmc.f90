program test_fullmc
    ...
    do ix = 0, nx-1
        do iy = 0, ny-1
            ...
            if (galb(ix,iy) < 0.0_dp .or. galb(ix,iy) > 1.0_dp) then
                write(dbgmsg, '(A,I4,A,I4,A,F12.6)') "Error: Invalid galb at (", ix, ",", iy, "): ", galb(ix,iy)
                write(*,*) trim(dbgmsg)
                stop
            else if (bgrnd(ix,iy) < 0.0_dp) then
                write(dbgmsg, '(A,I4,A,I4,A,F12.6)') "Error: Negative bgrnd at (", ix, ",", iy, "): ", bgrnd(ix,iy)
                write(*,*) trim(dbgmsg)
                stop
            end if
        end do
    ...
    if (wgttype == 1) then ! Absorption from the path length
        ...
        if (source <= 1) then
            ...
            recorder_boundary => recorder_bound_null
        end if
    else if (wgttype == 2) then ! Absorption at collision
        collision_calc => collision_calc2
        weight_calc => weight_calc2
        if (source <= 1) then
            recorder_scattering => recorder_scat1
            recorder_boundary => recorder_bound1
        else if (source == 2) then
    ...
    call photon_initloc(source, ix, iy, iz, ia, xarr, yarr, zarr, dx, dy, dz, nx, ny, nz, maxx, maxy, maxz, &
        dirview, transfer_mode, rloc, rindsrc, rind)
    !-- direction
...
! contains
    ...
    !         call photon_rroulette(vqllpb, 0.05_dp, 0.1_dp, survived)
    !         if (.not. survived) exit
    !     end do
