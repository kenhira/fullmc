...
if __name__ == '__main__':
    ...
    fmc1.viewmu = np.cos(np.radians(15.0))
    ...
    fmc2.viewmu = np.cos(np.radians(15.0))
    fmc2.viewphi = np.radians(-90.0)
    fmc2.run_mc()
    radimg2 = fmc2.read_result(kind='img')
    # radtracer2 = fmc2.read_result(kind='tracer1')

    fmc3 = copy.deepcopy(fmc)
    fmc3.viewmu = np.cos(np.radians(0.0))
    fmc3.viewphi = np.radians(0.0)
    fmc3.run_mc()
    ...
    fmc_i.derivative = 1
    fmc_i.kext[:, :, :] = fmc.kabs[:, :, :] + (fmc.kext[:, :, :] - fmc.kabs[:, :, :]) * 0.5

    fmc_i1 = copy.deepcopy(fmc_i)
    fmc_i1.viewmu = np.cos(np.radians(15.0))
    ...
    fmc_i2.viewmu = np.cos(np.radians(15.0))
    fmc_i2.viewphi = np.radians(-90.0)
    fmc_i2.run_mc()
    radimg_i2 = fmc_i2.read_result(kind='img')
    radtracer_i2 = fmc_i2.read_result(kind='tracer1')

    fmc_i3 = copy.deepcopy(fmc_i)
    fmc_i3.viewmu = np.cos(np.radians(0.0))
    fmc_i3.viewphi = np.radians(0.0)
    fmc_i3.run_mc()
    ...
    update[:, :, 0, :, :, :] = imgdiff1[:, :, np.newaxis, np.newaxis, np.newaxis] * gain1
    update[:, :, 1, :, :, :] = imgdiff2[:, :, np.newaxis, np.newaxis, np.newaxis] * gain2
    update[:, :, 2, :, :, :] = imgdiff3[:, :, np.newaxis, np.newaxis, np.newaxis] * gain3
    ...
    m1 = ax1.imshow(radimg_i1[:, :, 0], cmap='Greys_r', vmin=0., vmax=data_max)
    ax1.set_aspect('equal')
    ax1.set_xlabel('X (km)')
    ax1.set_ylabel('Y (km)')
    ax1.set_title('(a) Angle 1')
    ax2 = fig2.add_subplot(1, 3, 2)
    m2 = ax2.imshow(radimg_i2[:, :, 0], cmap='Greys_r', vmin=0., vmax=data_max)
    ax2.set_aspect('equal')
    ax2.set_xlabel('X (km)')
    # ax2.set_ylabel('Y (km)')
    ax2.set_yticklabels([])
    ...
    m3 = ax3.imshow(radimg_i3[:, :, 0], cmap='Greys_r', vmin=0., vmax=data_max)
    ax3.set_aspect('equal')
    ax3.set_xlabel('X (km)')
    # ax3.set_ylabel('Y (km)')
    ax3.set_yticklabels([])
    ...
    m1 = ax1.imshow(imgdiff1[:, :], cmap='RdBu_r', vmin=-0.5*data_max, vmax=0.5*data_max)
    ax1.set_aspect('equal')
    ax1.set_xlabel('X (km)')
    ax1.set_ylabel('Y (km)')
    ax1.set_title('(a) Angle 1')
    ax2 = fig3.add_subplot(1, 3, 2)
    m2 = ax2.imshow(imgdiff2[:, :], cmap='RdBu_r', vmin=-0.5*data_max, vmax=0.5*data_max)
    ax2.set_aspect('equal')
    ax2.set_xlabel('X (km)')
    # ax2.set_ylabel('Y (km)')
    ax2.set_yticklabels([])
    ...
    m3 = ax3.imshow(imgdiff3[:, :], cmap='RdBu_r', vmin=-0.5*data_max, vmax=0.5*data_max)
    ax3.set_aspect('equal')
    ax3.set_xlabel('X (km)')
    # ax3.set_ylabel('Y (km)')
    ax3.set_yticklabels([])
    ax3.set_title('(c) Angle 3')
    fig3.colorbar(m1, ax=[ax1,ax2, ax3], orientation='horizontal', label='Radiance Difference', pad=0.22, shrink=0.45)
    fig3.suptitle('Radiance difference (first-guess - true)')
    ...
    data_max = max(
        np.max(radtracer_i1[:, :, :, 0, :, 0] / radimg_i1[:, :, 0, np.newaxis, np.newaxis]), 
        np.max(radtracer_i2[:, :, :, 0, :, 0] / radimg_i2[:, :, 0, np.newaxis, np.newaxis]),
        np.max(radtracer_i3[:, :, :, 0, :, 0] / radimg_i3[:, :, 0, np.newaxis, np.newaxis])
    )
    for ix in range(nx):
        for iy in range(ny):
            data_plot = radtracer_i1[ix, iy, :, 0, :, 0] / radimg_i1[ix, iy, 0, np.newaxis, np.newaxis]
            data_plot2 = radtracer_i2[ix, iy, :, 0, :, 0] / radimg_i2[ix, iy, 0, np.newaxis, np.newaxis]
            data_plot3 = radtracer_i3[ix, iy, :, 0, :, 0] / radimg_i3[ix, iy, 0, np.newaxis, np.newaxis]
            ...
            fig.colorbar(m, ax=[ax1, ax2, ax3], orientation='horizontal', label='Sensitivity', pad=0.22, shrink=0.45)
    ...
    for ix in range(nx):
        for iy in range(ny):
            ...
            lc = LineCollection(cloud_segs, colors='k', linewidths=1.0)
            ax1.add_collection(lc)
            m = ax1.pcolormesh(xx*1e-3, zz*1e-3, data_plot.T, cmap='viridis', vmin=-1.5, vmax=1.5)
            ax1.set_aspect('equal')
            ax1.set_xlabel('X (km)')
            ax1.set_ylabel('Z (km)')
            ax1.set_title('(a) Angle 1')
            ...
            lc = LineCollection(cloud_segs, colors='k', linewidths=1.0)
            ax2.add_collection(lc)
            m = ax2.pcolormesh(xx*1e-3, zz*1e-3, data_plot2.T, cmap='viridis', vmin=-1.5, vmax=1.5)
            ax2.set_aspect('equal')
            ax2.set_xlabel('X (km)')
            ax2.set_yticklabels([])
            ax2.set_title('(b) Angle 2')
            ...
            lc = LineCollection(cloud_segs, colors='k', linewidths=1.0)
            ax3.add_collection(lc)
            m = ax3.pcolormesh(xx*1e-3, zz*1e-3, data_plot3.T, cmap='viridis', vmin=-1.5, vmax=1.5)
            ax3.set_aspect('equal')
            ax3.set_xlabel('X (km)')
            ax3.set_yticklabels([])
            ax3.set_title('(c) Angle 3')
    ...
    ax.set_xlabel('X (km)')
    ax.set_ylabel('Z (km)')
    pcm = ax.pcolormesh(xx*1e-3, zz*1e-3, data_plot.T, cmap='viridis', vmin=-1.5, vmax=1.5)
    ax.set_title('Average Update')
    fig.colorbar(pcm, ax=ax, orientation='horizontal', label='Sensitivity', pad=0.22, shrink=0.45)
