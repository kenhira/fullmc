module fullmc_funcs
    ...
    abstract interface
        ...
        subroutine col_calc_iface(ksca_grid, kabs_grid, kcol_grid)
            implicit none
            integer, parameter :: dp = selected_real_kind(15, 307)
            real(dp), intent(in) :: ksca_grid, kabs_grid
    ...
    subroutine collision_calc1(ksca_grid, kabs_grid, kcol_grid)
        implicit none
        real(dp), intent(in) :: ksca_grid, kabs_grid
        real(dp), intent(out) :: kcol_grid
        kcol_grid = ksca_grid
    end subroutine collision_calc1

    subroutine collision_calc2(ksca_grid, kabs_grid, kcol_grid)
        implicit none
        real(dp), intent(in) :: ksca_grid, kabs_grid
        real(dp), intent(out) :: kcol_grid
    ...
    subroutine photon_raytrace(rloc, rind, dirsol, itlpbmax, kext, xarr, yarr, zarr, nx, ny, nz, dxs, transfer_mode, &
        ...
        do while (itlpb < itlpbmax)
            if (debug > 0) then
                if (debug == 1) then
                    print *, "Local estimation step ", itlpb, " rlpbloc: ", rlpbloc, " rlpbind: ", rlpbind, " rlpbdir: ", rlpbdir, " vqllpb", vqllpb
                else
