...
if __name__ == "__main__":
    ...
    data_plotc = np.zeros((nx, nz), dtype=np.float64)
    data_plotc2 = np.zeros((nx, nz), dtype=np.float64)
    ...
    for file_les in files_les:
        ...
        data_plotc += (radirr1[:, indy, :, 1, 1, 0]) * wgt * absscale
        data_plotc2 += (radirr2[:, indy, :, 1, 1, 0]) * wgt * absscale
    ...
    print('Mean reflectance (ICA): {:.6e}'.format(np.mean(data_plotc[:, -1]) / np.mean(data_plotb[:, -1])))
    print('Mean reflectance (3D RT): {:.6e}'.format(np.mean(data_plotc2[:, -1]) / np.mean(data_plotb2[:, -1])))
    ...
    fig.savefig(f'out/{out_dir_base}/11_fconv_xz_comparison.png', dpi=300)


    data_max = max(np.max(data_plotc), np.max(data_plotc2))
    data_min = min(np.min(data_plotc), np.min(data_plotc2))
    data_abs = max(abs(data_min), abs(data_max))
    norm = mcolors.Normalize(vmin=0., vmax=data_max)
    ...
    # ax = fig.add_subplot(1, 2, 1)
    ax = fig.add_axes([0.1, 0.15, 0.35, 0.75])
    m = ax.pcolormesh(xx * 1e-3, zz * 1e-3, data_plotc.T, cmap='cividis', norm=norm)
    lc = LineCollection(segs, colors='k', linewidths=1.0)
    ax.add_collection(lc)
    ax.set_aspect('equal')
    ax.set_xlabel('X (km)')
    ...
    # ax2 = fig.add_subplot(1, 2, 2)
    ax2 = fig.add_axes([0.5, 0.15, 0.35, 0.75])
    m2 = ax2.pcolormesh(xx * 1e-3, zz * 1e-3, data_plotc2.T, cmap='cividis', norm=norm)
    lc = LineCollection(segs, colors='k', linewidths=1.0, label='雲')
    ax2.add_collection(lc)
    ax2.set_aspect('equal')
    ax2.set_xlabel('X (km)')
    ...
    # fig.tight_layout()
    fig.subplots_adjust(bottom=0.18)
    fig.savefig(f'out/{out_dir_base}/11_flux_xz_comparison.png', dpi=300)
